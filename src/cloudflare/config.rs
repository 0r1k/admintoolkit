...
/// Decrypted view of an account, used only in-memory to call the Cloudflare API.
#[derive(Debug, Clone)]
...
impl Config {
    ...
    pub fn upsert_account(&mut self, id: Option<&str>, label: String, api_token: String) -> io::Result<String> {
        let encrypted = secret::encrypt(&api_token)?;
        let now = now_iso();

        if let Some(id) = id {
            if let Some(a) = self.accounts.iter_mut().find(|a| a.id == id) {
                a.label = label;
                a.api_token_encrypted = encrypted;
                a.updated_at = now;
    ...
    pub fn with_secret(&self, id: &str) -> Option<AccountWithSecret> {
        ...
        let decrypted = secret::decrypt(&a.api_token_encrypted).ok()?;
        Some(AccountWithSecret { id: a.id.clone(), label: a.label.clone(), api_token: decrypted })
    }
}
