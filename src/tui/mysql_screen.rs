//! MySQL / MariaDB User Manager screen — save reusable connection profiles
//! (direct or via an SSH jump host) and manage users on the selected one.
...
impl AddUserModal {
    ...
    fn next_field(&mut self) {
        self.field = match self.field {
            AddField::Username => AddField::Host,
            AddField::Host => AddField::Password,
    ...
    fn prev_field(&mut self) {
        self.field = match self.field {
            ...
            AddField::Host => AddField::Username,
            AddField::Password => AddField::Host,
...
impl EditUserModal {
    ...
    fn next_field(&mut self) {
        self.field = match self.field {
            ...
            EditField::BtnChangePassword => EditField::GrantOn,
            EditField::GrantOn => EditField::Privileges,
            EditField::Privileges => EditField::BtnGrant,
            EditField::BtnGrant => EditField::BtnRevoke,
            EditField::BtnRevoke => EditField::BtnDrop,
    ...
    fn prev_field(&mut self) {
        self.field = match self.field {
            ...
            EditField::GrantOn => EditField::BtnChangePassword,
            EditField::Privileges => EditField::GrantOn,
            EditField::BtnGrant => EditField::Privileges,
            EditField::BtnRevoke => EditField::BtnGrant,
            EditField::BtnDrop => EditField::BtnRevoke,
...
enum Msg {
    Log(bool, String),
    UsersResult(Result<Vec<MysqlUser>, String>),
}

pub struct MysqlScreen {
    tab: Tab,
...
impl MysqlScreen {
    ...
    fn trigger_fetch_users(&mut self) {
        ...
        let tx = self.tx.clone();
        thread::spawn(move || {
            let result = (|| -> Result<Vec<MysqlUser>, String> {
        ...
        match key.code {
            ...
            KeyCode::Enter => match m.field {
                AddField::BtnCreate => {
                    ...
                    let host = m.host.value().trim().to_string();
                    let password = m.password.value().to_string();
                    if username.is_empty() || host.is_empty() || password.is_empty() {
                        self.error("Username, Host and Password are required");
                        return false;
                    }
                    let grant_db = {
                    ...
                    thread::spawn(move || {
                        let result = (|| -> Result<(), String> {
                            let mut conn = client::connect(&cfg)?;
                            let grant = grant_db.as_deref().map(|db| (db, privileges.as_slice()));
                            client::create_user(&mut conn, &username, &host, &password, grant)
                        })();
                        match result {
                            Ok(()) => {
                                let _ = tx.send(Msg::Log(true, format!("User {username}@{host} created")));
                            }
                            Err(e) => {
                                let _ = tx.send(Msg::Log(false, format!("Failed to create {username}@{host}: {}", one_line(&e))));
                            }
                        }
                        let result2 = (|| -> Result<Vec<MysqlUser>, String> {
                            let mut conn = client::connect(&cfg)?;
                            client::list_users(&mut conn)
                        })();
            ...
            KeyCode::Backspace => match m.field {
                ...
                AddField::Host => m.host.backspace(),
            ...
            KeyCode::End => match m.field {
                ...
                AddField::Host => m.host.end_of_line(),
    ...
    fn handle_edit_user_modal_key(&mut self, m: &mut EditUserModal, key: KeyEvent) -> bool {
        match key.code {
            ...
            KeyCode::Enter => match m.field {
                EditField::BtnChangePassword => {
                    ...
                    thread::spawn(move || {
                        let result = (|| -> Result<(), String> {
                            let mut conn = client::connect(&cfg)?;
                            client::change_password(&mut conn, &user, &host, &new_password)
                        })();
                        match result {
                            Ok(()) => {
                                let _ = tx.send(Msg::Log(true, format!("Password changed for {user}@{host}")));
                            }
                            Err(e) => {
                                let _ = tx.send(Msg::Log(false, format!("Failed to change password for {user}@{host}: {}", one_line(&e))));
                            }
                        }
                    });
                    return true;
                ...
                EditField::BtnDrop => {
                    ...
                    thread::spawn(move || {
                        let result = (|| -> Result<(), String> {
                            let mut conn = client::connect(&cfg)?;
                            client::drop_user(&mut conn, &user, &host)
                        })();
                        match result {
                            Ok(()) => {
                                let _ = tx.send(Msg::Log(true, format!("User {user}@{host} dropped")));
                            }
                            Err(e) => {
                                let _ = tx.send(Msg::Log(false, format!("Failed to drop {user}@{host}: {}", one_line(&e))));
                            }
                        }
                        let result2 = (|| -> Result<Vec<MysqlUser>, String> {
                            let mut conn = client::connect(&cfg)?;
                ...
                EditField::Privileges => m.privileges.toggle(),
                EditField::BtnGrant => {
                    let target = m.grant_db.value().trim().to_string();
                    if target.is_empty() {
                        self.error("Enter a Grant On target first (e.g. dbname, dbname.table, or * for all)");
                        return false;
                    }
                    let privileges = m.privileges.selected_items();
                    let label = self.users_tab.connection_input.value().trim().to_string();
                    ...
                    let tx = self.tx.clone();
                    thread::spawn(move || {
                        let priv_label = if privileges.is_empty() { "ALL PRIVILEGES".to_string() } else { privileges.join(", ") };
                        let result = (|| -> Result<(), String> {
                            let mut conn = client::connect(&cfg)?;
                            client::grant_privileges(&mut conn, &user, &host, &target, &privileges)
                        })();
                        match result {
                            Ok(()) => {
                                let _ = tx.send(Msg::Log(true, format!("Granted {priv_label} on {target} to {user}@{host}")));
                            }
                            Err(e) => {
                                let _ = tx.send(Msg::Log(false, format!("Failed to grant privileges to {user}@{host}: {}", one_line(&e))));
                            }
                        }
                    });
                    return true;
                ...
                EditField::BtnRevoke => {
                    ...
                    thread::spawn(move || {
                        ...
                        let result = (|| -> Result<(), String> {
                            let mut conn = client::connect(&cfg)?;
                            client::revoke_privileges(&mut conn, &user, &host, &target, &privileges)
                        })();
                        match result {
                            Ok(()) => {
                                let _ = tx.send(Msg::Log(true, format!("Revoked {priv_label} on {target} from {user}@{host}")));
                            }
                            Err(e) => {
                                let _ = tx.send(Msg::Log(false, format!("Failed to revoke privileges from {user}@{host}: {}", one_line(&e))));
                            }
                        }
                    });
                    return true;
            ...
            KeyCode::Char(' ') if m.field == EditField::Privileges => m.privileges.toggle(),
            KeyCode::Char(c) => match m.field {
                EditField::Password => m.password.insert(c),
                EditField::GrantOn => m.grant_db.insert(c),
                _ => {}
            },
            KeyCode::Backspace => match m.field {
                EditField::Password => m.password.backspace(),
                EditField::GrantOn => m.grant_db.backspace(),
                _ => {}
            },
            KeyCode::Delete => match m.field {
                EditField::Password => m.password.delete(),
                EditField::GrantOn => m.grant_db.delete(),
                _ => {}
            },
            KeyCode::Left => match m.field {
                EditField::Password => m.password.left(),
                EditField::GrantOn => m.grant_db.left(),
                EditField::Privileges => m.privileges.left(),
                _ => {}
            },
            KeyCode::Right => match m.field {
                EditField::Password => m.password.right(),
                EditField::GrantOn => m.grant_db.right(),
                EditField::Privileges => m.privileges.right(),
                _ => {}
            },
            KeyCode::Home => match m.field {
                EditField::Password => m.password.home(),
                EditField::GrantOn => m.grant_db.home(),
                _ => {}
            },
            KeyCode::End => match m.field {
                EditField::Password => m.password.end_of_line(),
                EditField::GrantOn => m.grant_db.end_of_line(),
                _ => {}
            },
            _ => {}
        }
    ...
    fn draw_connections(&self, f: &mut Frame, area: Rect) {
        ...
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "needs CREATE USER / DROP USER / GRANT privileges — e.g. root, or a dedicated admin user",
    ...
    fn draw_users(&self, f: &mut Frame, area: Rect) {
        ...
        .style(Style::default().bg(bg2()));
        let table_rows: Vec<Row> = ut.rows.iter().map(|u| Row::new(vec![Cell::from(u.user.clone()), Cell::from(u.host.clone())])).collect();
        let users_title = format!(" Users ({}) ", ut.rows.len());
        let table = Table::new(table_rows, [Constraint::Length(24), Constraint::Min(16)])
            .header(header)
    ...
    fn draw_add_user_modal(&self, f: &mut Frame, m: &AddUserModal, area: Rect) {
        ...
        // Content width inside the modal: border(2) + margin(2) either side.
        let content_width = width.saturating_sub(4);
        let priv_rows = priv_picker::rows_needed(&m.privileges, content_width);
        // Fixed rows: Username,spacer,Host,hint,spacer,Password,spacer,GrantDb,
        // hint,spacer,PrivLabel (11) + priv_rows + spacer,buttons,spacer,navhint (4).
        // Same collapse-to-zero-height trap as the connections form — this must
        // stay derived from priv_rows, not a fixed guess.
        let content_rows = 11 + priv_rows + 4;
        let height = (content_rows + 4).min(area.height.saturating_sub(2));
        let modal_area = centered_rect(width, height, area);
        f.render_widget(Clear, modal_area);
        let block = Block::default()
            .title(Span::styled(" Add MySQL User ", Style::default().fg(title_color())))
            .borders(Borders::ALL)
        ...
        let mut constraints = vec![
            ...
            Constraint::Length(1), // [5] Password
            Constraint::Length(1), // [6] spacer
            Constraint::Length(1), // [7] Grant DB
            Constraint::Length(1), // [8] hint
            Constraint::Length(1), // [9] spacer
            Constraint::Length(1), // [10] Privileges label
        ];
        for _ in 0..priv_rows {
            constraints.push(Constraint::Length(1)); // [11..11+priv_rows) picker rows
        }
        constraints.extend([
            Constraint::Length(1), // spacer
            Constraint::Length(1), // buttons
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![Span::styled("Host:     ", lbl()), input_span(&m.host, m.field == AddField::Host, false, fw)])),
            rows[2],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled("'%' = any host, 'localhost' = local only, or a specific IP", lbl()))),
            rows[3],
        );
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![Span::styled("Grant On: ", lbl()), input_span(&m.grant_db, m.field == AddField::GrantDb, false, fw)])),
            rows[7],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "empty/'*' = every DB, 'db' or 'db.*' = whole DB, 'db.table' = one table",
                lbl(),
            ))),
            rows[8],
        );
        f.render_widget(Paragraph::new(Line::from(Span::styled("Privileges (\u{2190}/\u{2192} move, Enter/Space toggle):", lbl()))), rows[10]);
        if priv_rows > 0 {
            let first = rows[11];
        ...
        let after_priv = 11 + priv_rows as usize;
        f.render_widget(
            Paragraph::new(Line::from(vec![
    ...
    fn draw_edit_user_modal(&self, f: &mut Frame, m: &EditUserModal, area: Rect) {
        ...
        let content_width = width.saturating_sub(4);
        let priv_rows = priv_picker::rows_needed(&m.privileges, content_width);
        // Fixed rows: NewPassword,spacer,ChangePasswordBtn,spacer,GrantOn,hint,
        // spacer,PrivLabel (8) + priv_rows + spacer,Grant/RevokeBtns,spacer,
        // Drop/CancelBtns,spacer,navhint (6). Must track priv_rows exactly —
        // an undersized Length collapses rows to zero height.
        let content_rows = 8 + priv_rows + 6;
        let height = (content_rows + 4).min(area.height.saturating_sub(2));
        let modal_area = centered_rect(width, height, area);
        f.render_widget(Clear, modal_area);
        let block = Block::default()
            .title(Span::styled(format!(" User {}@{} ", m.user, m.host), Style::default().fg(title_color())))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(accent()))
            .style(Style::default().bg(bg2()));
        let inner = block.inner(modal_area);
        ...
        let mut constraints = vec![
            ...
            Constraint::Length(1), // 1 spacer
            Constraint::Length(1), // 2 Change Password button
            Constraint::Length(1), // 3 spacer
            Constraint::Length(1), // 4 Grant On
        ...
        constraints.extend([
            Constraint::Length(1), // spacer
            Constraint::Length(1), // Grant/Revoke buttons
            Constraint::Length(1), // spacer
            Constraint::Length(1), // Drop/Cancel buttons
            Constraint::Length(1), // spacer
            Constraint::Length(1), // nav hint
            Constraint::Min(0),
        ]);
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![Span::styled("Grant On:     ", lbl()), input_span(&m.grant_db, m.field == EditField::GrantOn, false, fw)])),
            rows[4],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "e.g. dbname, dbname.table, or * for all — required for Grant/Revoke below",
                lbl(),
            ))),
            rows[5],
        );
        ...
        if priv_rows > 0 {
            let first = rows[8];
            let picker_area = Rect::new(first.x, first.y, first.width, priv_rows);
            priv_picker::draw(f, &m.privileges, m.field == EditField::Privileges, picker_area);
        }
        let after_priv = 8 + priv_rows as usize;
        f.render_widget(
            Paragraph::new(Line::from(vec![
                btn_span("Grant", m.field == EditField::BtnGrant),
                Span::raw("  "),
                btn_span("Revoke", m.field == EditField::BtnRevoke),
            ])),
            rows[after_priv + 1],
        );
        f.render_widget(
