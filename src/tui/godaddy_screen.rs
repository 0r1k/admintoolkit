//! GoDaddy DNS Manager screen — manage multiple GoDaddy API accounts
//! (secrets encrypted at rest), search/filter DNS records across all of
...
//! domain, one account's every domain, or — via "Fetch All Accounts" —
//! every domain of every account). Search / Account / Type act as a
//! client-side filter over `rows`, so a global cross-account search is
//! just: Fetch All Accounts once, then type into Search.
...
/// CAA `tag` values GoDaddy accepts.
...
impl AccountsTab {
    fn new() -> Self {
        Self {
            ...
            api_secret: Input::default(),
            field: AccField::Table,
        }
    }
    ...
    fn next_field(&mut self) {
        self.field = match self.field {
            ...
            AccField::Label => AccField::ApiKey,
            AccField::ApiKey => AccField::ApiSecret,
            AccField::ApiSecret => AccField::BtnSave,
    ...
    fn prev_field(&mut self) {
        self.field = match self.field {
            ...
            AccField::ApiKey => AccField::Label,
            AccField::ApiSecret => AccField::ApiKey,
            AccField::BtnSave => AccField::ApiSecret,
    ...
    fn clear_form(&mut self) {
        ...
        self.api_key = Input::default();
        self.api_secret = Input::default();
    }
}
...
impl RecordsTab {
    ...
    fn next_field(&mut self) {
        self.field = match self.field {
            RecField::Account => RecField::Domain,
            RecField::Domain => RecField::BtnFetch,
    ...
    fn prev_field(&mut self) {
        self.field = match self.field {
            ...
            RecField::Domain => RecField::Account,
            RecField::BtnFetch => RecField::Domain,
...
/// Weight/Port; CAA needs Flags/Tag instead) — this is what makes the
/// modal reshape itself around the selected type.
fn add_active_fields(type_name: &str) -> Vec<AddField> {
    let mut v = vec![AddField::Domain, AddField::Type, AddField::Name];
    match type_name {
...
impl AddModal {
    fn new(domain: String) -> Self {
        let field = if domain.is_empty() { AddField::Domain } else { AddField::Type };
        Self {
...
fn edit_active_fields(type_name: &str) -> Vec<EditField> {
    ...
    v.extend([EditField::Value, EditField::Ttl, EditField::BtnUpdate, EditField::BtnDelete, EditField::BtnCancel]);
    v
}
...
impl EditModal {
    fn new(r: &DnsRecord) -> Self {
        ...
        let tag_idx = r
            .tag
            .as_deref()
            .and_then(|t| CAA_TAGS.iter().position(|c| *c == t))
            .unwrap_or(0);
        Self {
            original: r.clone(),
            type_idx,
...
fn parse_modal_record(
    ...
    let ttl_val: i64 = ttl.value().trim().parse().map_err(|_| "TTL must be a number (seconds)".to_string())?;
    if ttl_val < 600 {
        return Err(format!("TTL must be at least 600 seconds (GoDaddy's minimum) — got {ttl_val}"));
    }
...
enum GdMsg {
    ...
    DomainsResult(Result<Vec<String>, String>),
}

pub struct GoDaddyScreen {
    tab: Tab,
    ...
    history_scroll: u16,
    tx: mpsc::Sender<GdMsg>,
    rx: mpsc::Receiver<GdMsg>,
}
...
impl GoDaddyScreen {
    ...
    pub fn tick(&mut self) {
        while let Ok(msg) = self.rx.try_recv() {
            match msg {
                GdMsg::Log(ok, line) => self.history.push((ok, line)),
                GdMsg::RecordsResult(Ok((records, domains, skipped))) => {
                    self.history.push((
                        true,
                        format!(
                            "Fetched {} record(s) across {} domain(s) ({} skipped)",
                            records.len(),
                            domains,
                            skipped
                        ),
                ...
                GdMsg::RecordsResult(Err(e)) => {
                    self.history.push((false, one_line(&e)));
                }
                GdMsg::DomainsResult(Ok(domains)) => {
                    self.records_tab.known_domains = domains;
                }
                GdMsg::DomainsResult(Err(e)) => {
                    self.history.push((false, format!("Failed to list domains: {}", one_line(&e))));
                }
            }
        }
    }
    ...
    fn spawn_domains_refresh(&self, account: AccountWithSecret) {
        let tx = self.tx.clone();
        thread::spawn(move || {
            let result = api::get_domains(&account);
            let _ = tx.send(GdMsg::DomainsResult(result));
        });
    }
        ...
        match key.code {
            KeyCode::Esc
                if !self.records_tab.account_dropdown_open
                    && !self.records_tab.domain_dropdown_open
    ...
    pub fn handle_mouse(&mut self, me: MouseEvent, area: Rect) {
        if self.modal.is_some() {
            ...
            // the session — indistinguishable from "the mouse just
            // doesn't work anymore". Any click dismisses it, same as
            // Enter/Esc/q already do.
    ...
    fn handle_accounts_mouse(&mut self, me: MouseEvent, area: Rect) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([Constraint::Min(0), Constraint::Length(15), Constraint::Length(6)])
            .split(area);
        ...
        if let Some(idx) = mouse::table_row_hit(x, y, chunks[0], 1, self.cfg.accounts.len(), self.accounts_tab.table_idx) {
            ...
            if let Some(a) = self.cfg.accounts.get(idx) {
                ...
                self.accounts_tab.api_key = Input::new(&a.api_key);
                self.accounts_tab.api_secret = Input::default();
            }
            return;
        }
        ...
        let rows2 = Layout::default()
            ...
            .constraints([
                ...
                Constraint::Length(1),
                Constraint::Length(1), // [2] API Key
                Constraint::Length(1),
                Constraint::Length(1), // [4] API Secret
                Constraint::Length(1),
                Constraint::Length(1),
                Constraint::Length(1),
        ...
        if let Some(i) = mouse::button_row_hit(x, y, rows2[8], &["Save", "New", "Delete", "Test Key"]) {
            match i {
                0 => self.trigger_save_account(),
                1 => self.accounts_tab.clear_form(),
                2 => self.trigger_delete_account(),
                _ => self.trigger_test_key(),
            }
            return;
        }
        if mouse::in_rect(rows2[0], x, y) {
            ...
            self.accounts_tab.field = AccField::ApiKey;
        } else if mouse::in_rect(rows2[4], x, y) {
            self.accounts_tab.field = AccField::ApiSecret;
        }
    }
    ...
    fn handle_records_mouse(&mut self, me: MouseEvent, area: Rect) {
        ...
        if mouse::in_rect(top_rows[2], x, y) {
            ...
            if x >= btn_x {
                ...
                self.records_tab.field = RecField::Domain;
                if !self.records_tab.known_domains.is_empty() {
                    self.records_tab.domain_idx = 0;
                    self.records_tab.domain_dropdown_open = true;
                }
            }
            return;
        }
        ...
        if let Some(idx) = mouse::table_row_hit(x, y, chunks[2], 1, visible_len, self.records_tab.selected_row) {
            ...
            self.records_tab.modal = Some(RecordModal::Edit(EditModal::new(&record)));
            return;
        }
    ...
    fn handle_add_modal_mouse(&mut self, m: &mut AddModal, me: MouseEvent, area: Rect) -> bool {
        ...
        let rows = Layout::default()
            ...
            .margin(1)
            .constraints([
                Constraint::Length(1), // 0 Domain
                Constraint::Length(1),
                Constraint::Length(1), // 2 Type/Name/TTL
                Constraint::Length(1),
        ...
        if m.domain_dropdown_open {
            if let Some((x, y)) = mouse::left_click(&me) {
                let matches = filtered_domains(&self.records_tab.known_domains, m.domain.value());
                let dd_x = rows[0].x + 8;
                let dd_area = Rect {
                    x: dd_x,
                    y: rows[0].y + 1,
                    width: 34u16.min(area.width.saturating_sub(dd_x)),
                    height: (matches.len() as u16 + 2).min(10),
                };
                if let Some(idx) = mouse::table_row_hit(x, y, dd_area, 0, matches.len(), m.domain_idx) {
                    if let Some(d) = matches.get(idx) {
                        m.domain = Input::new(d);
                    }
                }
                m.domain_dropdown_open = false;
            }
            return false;
        }
    ...
    fn handle_edit_modal_mouse(&mut self, m: &mut EditModal, me: MouseEvent, area: Rect) -> bool {
        ...
        let rows = Layout::default()
            ...
            .margin(1)
            .constraints([
                Constraint::Length(1), // 0 Type/Name/TTL
                Constraint::Length(1),
    ...
    fn handle_accounts_key(&mut self, key: KeyEvent) {
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if let Some(a) = self.cfg.accounts.get(at.table_idx) {
                        ...
                        at.api_key = Input::new(&a.api_key);
                        at.api_secret = Input::default();
                    }
                    return;
                }
                _ => {}
        ...
        match key.code {
            ...
            KeyCode::Enter => match at.field {
                ...
                AccField::BtnTest => self.trigger_test_key(),
                _ => self.accounts_tab.next_field(),
            },
            KeyCode::Char(c) => match at.field {
                AccField::Label => at.label.insert(c),
                AccField::ApiKey => at.api_key.insert(c),
                AccField::ApiSecret => at.api_secret.insert(c),
                _ => {}
            },
            KeyCode::Backspace => match at.field {
                AccField::Label => at.label.backspace(),
                AccField::ApiKey => at.api_key.backspace(),
                AccField::ApiSecret => at.api_secret.backspace(),
                _ => {}
            },
            KeyCode::Delete => match at.field {
                AccField::Label => at.label.delete(),
                AccField::ApiKey => at.api_key.delete(),
                AccField::ApiSecret => at.api_secret.delete(),
                _ => {}
            },
            KeyCode::Left => match at.field {
                AccField::Label => at.label.left(),
                AccField::ApiKey => at.api_key.left(),
                AccField::ApiSecret => at.api_secret.left(),
                _ => {}
            },
            KeyCode::Right => match at.field {
                AccField::Label => at.label.right(),
                AccField::ApiKey => at.api_key.right(),
                AccField::ApiSecret => at.api_secret.right(),
                _ => {}
            },
            KeyCode::Home => match at.field {
                AccField::Label => at.label.home(),
                AccField::ApiKey => at.api_key.home(),
                AccField::ApiSecret => at.api_secret.home(),
                _ => {}
            },
            KeyCode::End => match at.field {
                AccField::Label => at.label.end_of_line(),
                AccField::ApiKey => at.api_key.end_of_line(),
                AccField::ApiSecret => at.api_secret.end_of_line(),
                _ => {}
            },
            _ => {}
        }
    ...
    fn trigger_save_account(&mut self) {
        ...
        let api_key = self.accounts_tab.api_key.value().trim().to_string();
        let api_secret = self.accounts_tab.api_secret.value().trim().to_string();
        if label.is_empty() || api_key.is_empty() {
            self.modal = Some(("Error".into(), "Label and API Key are required".into()));
            return;
        }
        let existing_id = self
            .accounts_tab
            .selected
            .and_then(|i| self.cfg.accounts.get(i))
            .map(|a| a.id.clone());
        if existing_id.is_some() && api_secret.is_empty() {
            self.modal = Some((
                "Error".into(),
                "API Secret is required to update (re-enter it)".into(),
            ));
            return;
        }
        if existing_id.is_none() && api_secret.is_empty() {
            self.modal = Some(("Error".into(), "API Secret is required".into()));
            return;
        }

        match self.cfg.upsert_account(existing_id.as_deref(), label, api_key, api_secret) {
            Ok(_) => {
    ...
    fn trigger_delete_account(&mut self) {
        let Some(idx) = self.accounts_tab.selected.or_else(|| {
            if self.cfg.accounts.is_empty() {
                None
            } else {
                Some(self.accounts_tab.table_idx)
            }
        }) else {
    ...
    fn trigger_test_key(&mut self) {
        let api_key = self.accounts_tab.api_key.value().trim().to_string();
        let api_secret = if !self.accounts_tab.api_secret.value().trim().is_empty() {
            self.accounts_tab.api_secret.value().trim().to_string()
        } else if let Some(idx) = self.accounts_tab.selected {
            self.cfg
                .accounts
                .get(idx)
                .and_then(|a| self.cfg.with_secret(&a.id))
                .map(|a| a.api_secret)
                .unwrap_or_default()
        } else {
            String::new()
        ...
        if api_key.is_empty() || api_secret.is_empty() {
            self.modal = Some(("Error".into(), "API Key and Secret are required to test".into()));
            return;
        }
        let tx = self.tx.clone();
        thread::spawn(move || {
            let result = api::test_key(&api_key, &api_secret);
            if result.ok {
                let _ = tx.send(GdMsg::Log(true, format!("Key valid — sees {} domain(s)", result.domains_count)));
            } else {
                let _ = tx.send(GdMsg::Log(false, format!("Key test failed: {}", one_line(&result.message.unwrap_or_default()))));
            }
        });
    }
    ...
    fn handle_records_key(&mut self, key: KeyEvent) {
        ...
        if self.records_tab.domain_dropdown_open {
            self.handle_domain_dropdown_key(key);
            return;
        }
            ...
            if visible_len > 0 {
                match key.code {
                    ...
                    KeyCode::Char('y') => {
                        ...
                        self.history.push((
                            ok,
                            if ok { format!("Copied to clipboard: {value}") } else { "Couldn't access the clipboard".into() },
                        ));
                        return;
                    }
                    _ => {}
    ...
    fn handle_account_dropdown_key(&mut self, key: KeyEvent) {
        ...
        if let Some(label) = committed_label {
            if let Some(account) = self.account_by_label(&label) {
                self.spawn_domains_refresh(account);
            }
    ...
    fn handle_domain_dropdown_key(&mut self, key: KeyEvent) {
        let mut committed_domain = false;
        {
            let rt = &mut self.records_tab;
            let matches: Vec<String> = filtered_domains(&rt.known_domains, rt.domain.value());
            match key.code {
                KeyCode::Esc => rt.domain_dropdown_open = false,
                KeyCode::Tab => {
                    rt.domain_dropdown_open = false;
                    rt.next_field();
                }
                KeyCode::BackTab => {
                    rt.domain_dropdown_open = false;
                ...
                KeyCode::Down => {
                    if rt.domain_idx + 1 < matches.len() {
                        rt.domain_idx += 1;
                    }
                }
                KeyCode::Enter => {
                    if let Some(d) = matches.get(rt.domain_idx) {
                        rt.domain = Input::new(d);
                        committed_domain = true;
                    }
                    rt.domain_dropdown_open = false;
                }
                KeyCode::Char(c) => {
                ...
                KeyCode::Delete => rt.domain.delete(),
                KeyCode::Left => rt.domain.left(),
                KeyCode::Right => rt.domain.right(),
                KeyCode::Home => rt.domain.home(),
                KeyCode::End => rt.domain.end_of_line(),
                _ => {}
            }
        }
        if committed_domain {
    ...
    fn trigger_fetch_records(&mut self) {
        ...
        if self.records_tab.known_domains.is_empty() {
            self.spawn_domains_refresh(account.clone());
        }
        let domain = self.records_tab.domain.value().trim().to_string();
        let tx = self.tx.clone();
        thread::spawn(move || {
            let result = if domain.is_empty() {
                api::fetch_account_records(&account)
            } else {
                match api::get_domain_records(&account, &domain) {
                    Ok(records) => Ok((records, 1, 0)),
                    Err(e) => Err(e.message),
                }
            };
            let _ = tx.send(GdMsg::RecordsResult(result));
        });
    }

    /// Auto-loads every account's records the first time the Records tab
    /// is opened with nothing loaded yet, so domains just show up instead
    /// of requiring a manual "Fetch All Accounts" click every time. Only
    /// fires once per session (until `records_tab.rows` is populated) and
    /// stays silent — no accounts configured yet is a normal state while
    /// setting the tool up, not something worth an error popup on every
    /// tab switch.
    ...
    fn trigger_fetch_all_accounts(&mut self) {
        ...
        let accounts: Vec<AccountWithSecret> = self
            .cfg
            .accounts
            .iter()
            .filter_map(|a| self.cfg.with_secret(&a.id))
            .collect();
        let tx = self.tx.clone();
        thread::spawn(move || {
            let (records, domains, skipped, errors) = api::fetch_all_accounts(accounts);
            for (label, msg) in &errors {
                let _ = tx.send(GdMsg::Log(false, format!("[{label}] {}", one_line(msg))));
            }
            let _ = tx.send(GdMsg::RecordsResult(Ok((records, domains, skipped))));
        });
    }
    ...
    fn trigger_open_add_modal(&mut self) {
        ...
        let Some(account) = self.account_by_label(&label) else {
            self.modal = Some((
                "Error".into(),
                "Select a valid account first (Account field above, use the dropdown)".into(),
            ));
            return;
        };
        if self.records_tab.known_domains.is_empty() {
        ...
        let domain = self.records_tab.domain.value().trim().to_string();
        self.records_tab.modal = Some(RecordModal::Add(AddModal::new(domain)));
    }
    ...
    fn handle_add_modal_domain_dropdown_key(&mut self, m: &mut AddModal, key: KeyEvent) {
        let matches: Vec<String> = filtered_domains(&self.records_tab.known_domains, m.domain.value());
        match key.code {
            KeyCode::Esc => m.domain_dropdown_open = false,
            KeyCode::Tab => {
                m.domain_dropdown_open = false;
                m.next_field();
            }
            ...
            KeyCode::Down => {
                if m.domain_idx + 1 < matches.len() {
                    m.domain_idx += 1;
                }
            }
            KeyCode::Enter => {
                if let Some(d) = matches.get(m.domain_idx) {
                    m.domain = Input::new(d);
                }
                m.domain_dropdown_open = false;
            }
            ...
            KeyCode::Delete => m.domain.delete(),
            KeyCode::Left => m.domain.left(),
            KeyCode::Right => m.domain.right(),
            KeyCode::Home => m.domain.home(),
            KeyCode::End => m.domain.end_of_line(),
            _ => {}
        }
    }
    ...
    fn handle_add_modal_key(&mut self, m: &mut AddModal, key: KeyEvent) -> bool {
        if m.domain_dropdown_open {
            self.handle_add_modal_domain_dropdown_key(m, key);
            return false;
        }

        let is_discard_attempt =
            key.code == KeyCode::Esc || (key.code == KeyCode::Enter && m.field == AddField::BtnCancel);
        ...
        match key.code {
            ...
            KeyCode::Enter => match m.field {
                ...
                AddField::BtnStage => {
                    match parse_modal_record(m.type_idx, &m.name, &m.priority, &m.weight, &m.port, &m.flags, m.tag_idx, &m.value, &m.ttl)
                    {
                        Ok(input) => {
                ...
                AddField::BtnSaveAll => {
                    let domain = m.domain.value().trim().to_string();
                    if domain.is_empty() {
                        self.modal = Some(("Error".into(), "Pick or type a domain first".into()));
                        return false;
                    }
                    ...
                    let domain_for_refresh = domain.clone();
                    thread::spawn(move || {
                        let total = batch.len();
                        let mut ok_count = 0usize;
                        for input in &batch {
                            match api::add_record(&account, &domain, input) {
                                Ok(()) => ok_count += 1,
                                Err(e) => {
                                    let _ = tx.send(GdMsg::Log(
                                        false,
                                        format!("[{domain}] failed to add {} {}: {}", input.type_, input.subdomain, one_line(&e)),
                                    ));
                                }
                            }
                        }
                        let _ = tx.send(GdMsg::Log(ok_count == total, format!("[{domain}] added {ok_count}/{total} record(s)")));
                        let result = match api::get_domain_records(&account_for_refresh, &domain_for_refresh) {
                            Ok(records) => Ok((records, 1, 0)),
                            Err(e) => Err(e.message),
                        };
                        let _ = tx.send(GdMsg::RecordsResult(result));
            ...
            KeyCode::Backspace => match m.field {
                AddField::Domain => m.domain.backspace(),
            ...
            KeyCode::Delete => match m.field {
                AddField::Domain => m.domain.delete(),
            ...
            KeyCode::Right => match m.field {
                AddField::Domain => m.domain.right(),
            ...
            KeyCode::End => match m.field {
                AddField::Domain => m.domain.end_of_line(),
    ...
    fn handle_edit_modal_key(&mut self, m: &mut EditModal, key: KeyEvent) -> bool {
        ...
        match key.code {
            ...
            KeyCode::Enter => match m.field {
                ...
                EditField::BtnUpdate => match parse_modal_record(
                    m.type_idx,
                    &m.name,
                    &m.priority,
                    &m.weight,
                    ...
                    Ok(input) => {
                        ...
                        let domain = old.domain.clone();
                        let tx = self.tx.clone();
                        let account_for_refresh = account.clone();
                        let domain_for_refresh = domain.clone();
                        let refresh_domain = self.records_tab.domain.value().trim().to_string();
                        thread::spawn(move || {
                            let msg = match api::update_record(&account, &domain, &old, &input) {
                                Ok(()) => (true, format!("[{domain}] updated {} {}", input.type_, input.subdomain)),
                                Err(e) => (false, format!("[{domain}] failed to update {}: {}", input.type_, one_line(&e))),
                            };
                            let _ = tx.send(GdMsg::Log(msg.0, msg.1));
                            let fetch_domain = if refresh_domain.is_empty() { domain_for_refresh } else { refresh_domain };
                            let result = match api::get_domain_records(&account_for_refresh, &fetch_domain) {
                                Ok(records) => Ok((records, 1, 0)),
                                Err(e) => Err(e.message),
                            };
                            let _ = tx.send(GdMsg::RecordsResult(result));
                ...
                EditField::BtnDelete => {
                    ...
                    let refresh_domain = self.records_tab.domain.value().trim().to_string();
                    thread::spawn(move || {
                        let msg = match api::delete_record(&account, &domain, &old.type_, &old.subdomain, &old.value) {
                            Ok(()) => (true, format!("[{domain}] deleted {} {}", old.type_, old.subdomain)),
                            Err(e) => (false, format!("[{domain}] failed to delete {}: {}", old.type_, one_line(&e))),
                        };
                        let _ = tx.send(GdMsg::Log(msg.0, msg.1));
                        let fetch_domain = if refresh_domain.is_empty() { domain_for_refresh } else { refresh_domain };
                        let result = match api::get_domain_records(&account_for_refresh, &fetch_domain) {
    ...
    fn draw_accounts(&self, f: &mut Frame, area: Rect) {
        ...
        let rows: Vec<Row> = self
            ...
            .map(|a| {
                Row::new(vec![
                    ...
                    Cell::from(a.api_key.clone()),
                    Cell::from("\u{2022}\u{2022}\u{2022}\u{2022}\u{2022}\u{2022} (encrypted)"),
                ])
            })
            .collect();

        let table = Table::new(rows, [Constraint::Length(20), Constraint::Length(24), Constraint::Min(0)])
            .header(header)
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                input_span(&at.label, at.field == AccField::Label, false, fw),
            ])),
            rows2[0],
        );
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("API Key:      ", lbl()),
                input_span(&at.api_key, at.field == AccField::ApiKey, false, fw),
            ])),
            rows2[2],
        );
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("API Secret:   ", lbl()),
                input_span(&at.api_secret, at.field == AccField::ApiSecret, true, fw),
            ])),
            rows2[4],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                if at.selected.is_some() {
                    "editing existing account — re-enter secret to update it"
                } else {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::raw("  "),
                btn_span("Test Key", at.field == AccField::BtnTest),
            ])),
            rows2[8],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "\u{2191}\u{2193} select row  Enter load row  Tab navigate  Esc back",
                lbl(),
            ))),
            rows2[10],
        );
    ...
    fn draw_records(&self, f: &mut Frame, area: Rect) {
        ...
        let top_rows = Layout::default()
            ...
            .margin(1)
            .constraints([
                Constraint::Length(1), // [0] Account
                Constraint::Length(1), // [1] spacer
                Constraint::Length(1), // [2] Domain + Fetch
            ])
            .split(top_inner);
        let fw = top_rows[0].width.saturating_sub(12) as usize;
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Account:    ", lbl()),
                input_span(&rt.account_input, rt.field == RecField::Account, false, fw),
            ])),
            top_rows[0],
        );
        let fw2 = top_rows[2].width.saturating_sub(12 + 2 + 16) as usize;
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Domain:     ", lbl()),
                input_span(&rt.domain, rt.field == RecField::Domain, false, fw2),
                Span::raw("  "),
        ...
        let search_rows = Layout::default()
            ...
            .margin(1)
            .constraints([
                Constraint::Length(1), // [0] Search
                Constraint::Length(1), // [1] spacer
                Constraint::Length(1), // [2] Type + Fetch All
            ])
            .split(search_inner);
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                input_span(&rt.search, rt.field == RecField::Search, false, search_fw),
            ])),
            search_rows[0],
        );
        f.render_widget(
        ...
        let action_rows = Layout::default()
            ...
            .split(action_inner);
        f.render_widget(
            Paragraph::new(Line::from(vec![btn_span("Add Record(s)", rt.field == RecField::BtnAddRecords)])),
            action_rows[0],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "Enter on a table row opens Edit/Delete  \u{2022}  y copies its Value  \u{2022}  Add lets you pick the domain inside",
                lbl(),
            ))),
            action_rows[1],
        );
        ...
        if rt.domain_dropdown_open {
            render_dropdown(f, &filtered_domains(&rt.known_domains, rt.domain.value()), rt.domain_idx, top_rows[2], 12, area);
        }
    }
    ...
    fn draw_add_modal(&self, f: &mut Frame, m: &AddModal, area: Rect) {
        ...
        let domain_fw = rows[0].width.saturating_sub(9) as usize;
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Domain: ", lbl()),
                input_span(&m.domain, m.field == AddField::Domain, false, domain_fw),
            ])),
            rows[0],
        );
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Value: ", lbl()),
                input_span(&m.value, m.field == AddField::Value, false, value_fw),
            ])),
            rows[6],
        );
        f.render_widget(
            Paragraph::new(Line::from(vec![btn_span("+ Add to list", m.field == AddField::BtnStage)])),
            rows[8],
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(format!("Pending ({}):", m.pending.len()), lbl()))),
            rows[10],
        );
        ...
        if m.pending.is_empty() {
            f.render_widget(
                Paragraph::new(Line::from(Span::styled(
                    "  (none yet — pick a domain, fill the form above, and \"+ Add to list\")",
                    lbl(),
                ))),
                rows[11],
            );
            ...
            let lines: Vec<Line> = m
                ...
                .map(|(i, r)| {
                    ...
                    Line::from(Span::styled(
                        format!("{prefix}{:<6} {:<16} {:<28} {}", r.type_, r.subdomain, r.value, r.ttl),
                        style,
                    ))
                })
                .collect();
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                btn_span("Save All", m.field == AddField::BtnSaveAll),
                Span::raw("  "),
        ...
        if m.domain_dropdown_open {
            render_dropdown(f, &filtered_domains(&self.records_tab.known_domains, m.domain.value()), m.domain_idx, rows[0], 8, area);
        } else if self.records_tab.known_domains.is_empty() && m.field == AddField::Domain {
    ...
    fn draw_edit_modal(&self, f: &mut Frame, m: &EditModal, area: Rect) {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Value: ", lbl()),
                input_span(&m.value, m.field == EditField::Value, false, value_fw),
            ])),
            rows[4],
        );
        f.render_widget(
...
fn draw_type_specific_add_row(f: &mut Frame, m: &AddModal, area: Rect) {
    let line = match m.current_type() {
        "MX" => Some(Line::from(vec![
            Span::styled("Priority: ", lbl()),
            input_span(&m.priority, m.field == AddField::Priority, false, 8),
            Span::styled("  (lower number = higher priority)", lbl()),
        ])),
...
fn draw_type_specific_edit_row(f: &mut Frame, m: &EditModal, area: Rect) {
    let line = match m.current_type() {
        "MX" => Some(Line::from(vec![
            Span::styled("Priority: ", lbl()),
            input_span(&m.priority, m.field == EditField::Priority, false, 8),
            Span::styled("  (lower number = higher priority)", lbl()),
        ])),
        "SRV" => Some(Line::from(vec![
            Span::styled("Priority: ", lbl()),
...
/// Shared dropdown-list overlay renderer used by the Account and Domain
...
fn filtered_domains(known: &[String], query: &str) -> Vec<String> {
    let q = query.to_lowercase();
    known
        .iter()
