...
mod tests_fixtures {
    use super::*;
    use crate::config_check::format::{self, Format};
    use std::fs;

    /// `expect_fixable`: `true` means `try_fix` must return `Some` *and*
    /// the result must validate; `false` means it must return `None` —
    /// not just "returned something that's still invalid". A fixer that
    /// changes bytes without fixing anything (e.g. by accident, like the
    /// trailing-newline bug this test caught once) would still trigger a
    /// pointless write-and-backup in the real flow, so "no applicable fix"
    /// must mean `None`, not "changed but still broken".
    fn check(path: &str, format: Format, expect_fixable: bool) {
        let content = fs::read_to_string(path).unwrap();
        let err = format::validate(format, &content).unwrap_err();
        println!("{path}: INVALID as expected — {}", err.lines().next().unwrap_or(""));
        match try_fix(format, &content) {
            Some(fixed) => {
                let now_ok = format::validate(format, &fixed).is_ok();
                println!("{path}: fixer changed content, now valid = {now_ok}");
                assert!(expect_fixable, "{path}: fixer changed content but this fixture should have been left untouched (now_ok={now_ok})");
                assert!(now_ok, "{path}: fixer changed content but it's still invalid");
            }
            None => {
                println!("{path}: fixer made no changes");
                assert!(!expect_fixable, "{path}: expected a fix but fixer did nothing");
            }
        }
    }

    #[test]
    fn fixtures_behave_as_documented() {
        check("tests/json-trailing-comma.json", Format::Json, true);
        check("tests/json-missing-colon.json", Format::Json, false);
        check("tests/toml-unterminated-string.toml", Format::Toml, false);
        check("tests/toml-missing-equals.toml", Format::Toml, false);
        check("tests/yaml-tab-indent.yaml", Format::Yaml, true);
        check("tests/yaml-unclosed-flow.yml", Format::Yaml, false);
        check("tests/xml-bare-ampersand.xml", Format::Xml, true);
        check("tests/xml-mismatched-tag.xml", Format::Xml, false);
    }
}
